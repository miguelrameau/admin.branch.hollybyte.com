...
HB2.admin.playlist = ( function($) {
    ...
    function onClickMenuTab() {
        if(playlistGeneralTab[0] ? true : false)
            playlistGeneralTab[0].setAttribute("class", "program-right-tabs-nSel");
        if(playlistAssetTab[0] ? true : false)
            playlistAssetTab[0].setAttribute("class", "program-right-tabs-nSel");
        if(playlistMetaTab[0] ? true : false)
            playlistMetaTab[0].setAttribute("class", "program-right-tabs-nSel");
        ...
        switch(selectTab.id) {
            case 'tabGeneral':
                playlistGeneralTab[0].setAttribute("class", "program-right-tabs-Sel");
            ...
            case 'tabAssets':
                ...
                playlistAssetTab[0].setAttribute("class", "program-right-tabs-Sel");
                playlistAssetContent[0].style.display = "block";
                break;

            case 'tabMetadatas':
                playlistMetaTab[0].setAttribute("class", "program-right-tabs-Sel");
    ...
    function initPlaylistValuesTable(objs) {
        HB2.admin.metavalue.init(playlistValuesTable, objs, 'repo1.eu.hollybyte.com');
    }
    ...
    function initAssetTable(table, values) {
        ...
        for(var i = 0; i < values.length; i++) {
            var data = [];
            var obj = values[i];
            data[0] = '<input class="HB-Playlist-checkbox" type="checkbox" id="' + obj.playlist + '" />';
            ...
            data[3] = '<img class="HB-Playlist-delAsset" src="/images/common/delete.png" alt="delete-asset" />';
        ...
        $('.HB-Playlist-delAsset').click(function() {
            onClickRemoveAsset(this, table);
        });
        $('.HB-Playlist-checkbox').click(function() {
            checkStatus(this);
        });
        return table;
    }
    ...
    function updateAssetTable(table, values) {
        ...
        for(var i = 0; i < values.length; i++) {
            var data = [];
            var obj = values[i];
            data[0] = '<input class="HB-Playlist-checkbox" type="checkbox" />';
            if(!HB2.admin.isEmpty(obj.splash)) {
    ...
    function setJSONAssets(assets) {
        if(!jQuery.isEmptyObject(assets)) {
            //console.info('setJsonAssets assets: '+assets);
    ...
    function setJSONPlaylist(obj) {
        ...
        if(!HB2.admin.isEmpty(obj.splash)) {
            ...
            $('#splash').attr("src", "http://" + repo + "/" + splash);
        }
        currVal = obj;
    }
