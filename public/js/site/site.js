...
HB2.admin.site = ( function($) {
    ...
    function renderSiteAssetConnectors(obj) {
        ...
        html += '<img src="https://hollybyte.s3.amazonaws.com/sites/admin/img/connector/iframe.png" />';
        html += '<span>Iframe</span>'
        html += '</div>';
        html += '<div class="connector_embed">';
        html += '<img src="https://hollybyte.s3.amazonaws.com/sites/admin/img/connector/embed.png" />';
    ...
    function initAssetsTable(objs) {
        ...
        for(var i = 0; i < objs.length; i++) {
            ...
            if(objs[i].assetObj.splash != 'undefined') {
                row[0] = '<img src="https://' + repo + '/' + objs[i].assetObj.splash + '" alt="splash" width="90px" height="50px">';
            } else {
                // TODO ERROR
            }
        ...
        $(assetsTable.fnGetNodes()).click(function() {
            onClickTable();
        });
    };
    ...
    function load() {
        ...
        loadConnectors();
        for(i in playlists) {
            $('#sitePlaylist').append($("<option></option>").attr("value", playlists[i].id).text(playlists[i].id));
        }
        for(i in players) {
            $('#siteChannelPlayers').append($("<option></option>").attr("value", players[i].id).text(players[i].id + ' (' + players[i].type + ')'));
        }
    ...
    function getSiteAssets() {
        $.each(assetsTable.fnGetNodes(), function(index, value) {
            var channels = [];
            var size = value.cells[2].childNodes[0].childNodes.length;
            for(var i = 0; i < size; i++) {
                console.log('div: ', value.cells[2].childNodes[0].childNodes[i]);
                console.log('checked: ', value.cells[2].childNodes[0].childNodes[i].childNodes[0].checked);
                if(value.cells[2].childNodes[0].childNodes[i].childNodes[0].checked) {
                    channels.push(value.cells[2].childNodes[0].childNodes[i].childNodes[0].value);
                    console.log('channel: ', value.cells[2].childNodes[0].childNodes[i].childNodes[0].value);
                }
            }
            siteAssets[index].channels = channels;
        });
        return siteAssets;
    }
    ...
    function onClickSocialPost() {
        ...
        if(currSocial.type === "FACEBOOK") {
            ...
            var swf = "http://" + repo + "/acc/" + pub.account + "/st/" + site.id + "/" + site.channels[0].name + "/public/swf/flowplayer.commercial-3.2.7.swf";
    ...
    function initSiteEdit() {
        ...
        $('#siteClearBtn').click(function() {
            ...
            validator.resetForm();
            var regionName = HB2.admin.getRegionName(pub.region);
            var domain = pub.account + '.hollybyte.tv';
            $('#siteRegionTxt').html(regionName);
            $('#siteRegion').val(pub.region);
        ...
        var regionName = HB2.admin.getRegionName(pub.region);
        var domain = pub.account + '.hollybyte.tv';
        repo = getRepoByRegion(HB2.admin.site.region);
        ...
        $('#siteId').keyup(function() {
            var domain = $(this).val() + '.' + pub.account + '.hollybyte.tv';
            $('#siteDomainTxt').html(domain);
            $('#siteDomain').val(domain);
        });
