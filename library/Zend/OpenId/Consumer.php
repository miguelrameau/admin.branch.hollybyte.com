...
class Zend_OpenId_Consumer {
        ...
        if($this->_storage->getAssociationByHandle($params['openid_assoc_handle'], $url, $macFunc, $secret, $expires)) {
            ...
            if(base64_decode($params['openid_sig']) == Zend_OpenId::hashHmac($macFunc, $data, $secret)) {
                if(!Zend_OpenId_Extension::forAll($extensions, 'parseResponse', $params)) {
                    $this->_setError("Extension::parseResponse failure0");
                    return false;
                }
                ...
                if(isset($params['openid_claimed_id'])) {
                    ...
                    if(!Zend_OpenId::normalize($id)) {
                        ...
                        $this->_setError("Discovery information verification failed1");
                        return false;
                    }
                }
                return true;
            ...
            if((isset($params['openid_identity']) && $params["openid_identity"] != $id) || (isset($params['openid_op_endpoint']) && $params['openid_op_endpoint'] != $server) || $discovered_version != $version) {
                $this->_setError("Discovery information verification failed2");
                return false;
            }
            ...
            if(isset($ret['is_valid']) && $ret['is_valid'] == 'true') {
                if(!Zend_OpenId_Extension::forAll($extensions, 'parseResponse', $params)) {
                    $this->_setError("Extension::parseResponse failure1");
                    return false;
                }
                return true;
            }
    ...
    protected function _associate($url, $version, $priv_key = null) {
        ...
        if($version >= 2.0) {
            $params = array('openid.ns' => Zend_OpenId::NS_2_0,
                'openid.mode' => 'associate',
                'openid.assoc_type' => 'HMAC-SHA256',
                'openid.session_type' => 'DH-SHA256', );
        } else {
            $params = array('openid.mode' => 'associate',
    ...
    protected function _httpRequest($url, $method = 'GET', array $params = array(), &$status = null, $includeHeader = false) {
        ...
        if($client === null) {
            $client = new Zend_Http_Client($url, array('maxredirects' => 4,
                'timeout' => 15,
    ...
    protected function _discoveryHtml(&$id, &$server, &$version) {
        $this->_log->info('[Consumer][_discoveryHtml] ID: '.$id);
        $realId = $id;
    ...
    protected function _discovery(&$id, &$server, &$version) {
        $this->_log->info('[Consumer][_discovery] ID: '.$id);
        $realId = $id;
        ...
        if(in_array(substr($id, 0, 1), $xriChars)) { //It is an XRI
            $xrds = $this->_httpRequest("https://xri.net/".$id, "GET", array('query' => '',
                '_xrd_r' => 'application/xrds+xml'), $status);
            if($status != 200 || !is_string($response)) {
                return false;
            }
    ...
    protected function _discoveryXrds($xrds, &$realId, &$server, &$version) {
        $this->_log->info('[Consumer][_discoveryXrds] XRDS: '.$xrds);
        $this->_log->info('[Consumer][_discoveryXrds] RealID: '.$realId);
            ...
            foreach($XRD->Service as $service) { //Cycle through the different services described in this XRDS document
                ...
                foreach($service->Type as $type) { //Look if this service has the right type (eg an OpenId descriptor) and if so, store the information appropriately
                    if("http://specs.openid.net/auth/2.0/server" == $type[0]) {
                        $this->_log->info('[Consumer][_discoveryXrds] http://specs.openid.net/auth/2.0/server');
                        ...
                        $serverServices[$priority][] = array("protocol" => "http://specs.openid.net/auth/2.0/server",
                            "endpoints" => $uris,
                            "claimedID" => "http://specs.openid.net/auth/2.0/identifier_select",
                            "localID" => "http://specs.openid.net/auth/2.0/identifier_select");
                        $version = 2.0;
                        continue 2;
                    } elseif(("http://specs.openid.net/auth/2.0/signon" == $type[0]) || (($version != 2.0) && ("http://openid.net/signon/1.1" == $type[0]))) {
                        $this->_log->info('[Consumer][_discoveryXrds] http://specs.openid.net/auth/2.0/signon');
                        ...
                        $signonServices[$priority][] = array("protocol" => $type[0],
                            "endpoints" => $uris,
                            "claimedID" => $id,
                            "localID" => (isset($XRD->localID[0]) ? strval($XRD->localID[0]) : $id));
        ...
        $this->_log->info('[Consumer][_discoveryXrds] services: '.print_r($services, TRUE));
        ...
        $this->_log->info('[Consumer][_discoveryXrds] Server: '.$server);
        $this->_log->info('[Consumer][_discoveryXrds] RealId: '.$realId);
        return true;
    }
    ...
    protected function _discoveryYadis($userInput, &$realId, &$server, &$version) {
        $this->_log->info('[Consumer][_discoveryYadis] UsetInput: '.$userInput);
        $this->_log->info('[Consumer][_discoveryYadis] RealID: '.$realId);
        $status = null;
        $resp = $this->_httpRequest($userInput, "GET", array(), $status, true);
        //We need HTTP header information as well
        $n = strpos($resp, "\n\n");
        ...
        $xrds = null;
        $this->_log->info('[Consumer][_discoveryYadis] Headers: '.print_r($header, TRUE));
    ...
    protected function _checkId($immediate, $id, $returnTo = null, $root = null, $extensions = null, Zend_Controller_Response_Abstract $response = null) {
        $this->_log->info('[Consumer][_checkId] ID: '.$id);
        $this->_setError('');
        ...
        if($version <= 2.0) {
            if($this->_session !== null) {
                ...
                $_SESSION["zend_openid"] = array("identity" => $id,
        ...
        $params['openid.realm'] = 'https://'.$_SERVER['SERVER_NAME'].'/';

        $this->_log->info('[Consumer][_checkId] Params: '.print_r($params, TRUE));
